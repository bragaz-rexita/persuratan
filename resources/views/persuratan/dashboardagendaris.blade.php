					...
					<div class="card card-widget widget-user">
                        ...
                        <div class="card-footer">
							<ul class="nav nav-pills flex-column">
								...
								<li class="nav-item"><a href="#" class="nav-link btnopensuratmasuk"><i class="fa fa-envelope"></i> Tambah Surat Masuk<span class="badge badge-primary float-right countsuratmasuk">0</span></a></li>
...
<div class="modal fade" id="modalaprovalnomor">
	<div class="modal-dialog modal-xl">
		<div class="modal-content">
		<div class="modal-header">
			<h4 class="modal-title">Persetujuan Permohonan Nomor Staf</h4>
			<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
		</div>
		<div class="modal-body">
			<div class="form-group">
				<label class="col-form-label">Pemohon<span class="text-danger">*</span></label>
				<input type="text" class="form-control" id="reqnomor_pemohon" name="reqnomor_pemohon" disabled="disable">
			</div>
			<div class="form-group">
				<div class="row">
					<div class="col-lg-2 col-md-2">
						<label for="reqnomor_nomor">Nomor</label>
						<input type="text" class="form-control" id="reqnomor_nomor" name="reqnomor_nomor" disabled="disable">
					</div>
					<div class="col-lg-2 col-md-2">
						<label for="reqnomor_tanggal">Tanggal</label>
						<input type="text" class="form-control" id="reqnomor_tanggal" name="reqnomor_tanggal" disabled="disable">
					</div>
					<div class="col-lg-8 col-md-8">
						<label for="reqnomor_perihal">Perihal</label>
						<input type="text" class="form-control" id="reqnomor_perihal" name="reqnomor_perihal" disabled="disable">
					</div>
				</div>
			</div>
			<div class="form-group">
				<div class="row">
					<div class="col-lg-6">
						<label for="reqnomor_jenissrt">Jenis Surat</label>
						<select id="reqnomor_jenissrt" name="reqnomor_jenissrt" size="1" class="form-control">
							<option value="BIASA">Surat Biasa (TTD)</option>
							<option value="Nota Dinas">Nota Dinas (TTD)</option>
							<option value="UPLOAD">Surat dengan TTE</option>
							<option value="UPLQRMAN">Surat dengan Tanda Tangan Elektronik (Dengan QrCode Manual)</option>
					...
					<div class="col-lg-6">
						<label for="reqnomor_kodepjbt">Penandatangan</label>
						<select id="reqnomor_kodepjbt" name="reqnomor_kodepjbt" size="1" class="form-control">
							<option value="">Pilih Salah Satu</option>
								@foreach($pejabats as $rpejabats)
									<option value="{{ $rpejabats['kode'] }}">{{ $rpejabats['pejabat'] }}</option>
								@endforeach
		...
		<div class="modal-footer justify-content-between">
        	<input type="hidden" class="form-control" id="reqnomor_idne" name="reqnomor_idne">
			<button type="button" class="btn btn-success pull-right" id="btnapprovereqnomor">Approve</button>
			<button type="button" class="btn btn-warning pull-left" data-dismiss="modal">Close</button>	
		</div>
		</div><!-- /.modal-content -->
	</div><!-- /.modal-dialog -->
...
$(document).ready(function () {
	...
	//KELOMPOK_SURAT_KELUAR
		$('.btnopensuratkeluar').click(function () {
			...
			$("#gridsuratkeluar").jqxGrid({
				...
				altrows			: true,
				columns			: [
					{ text: 'Edit/Replace', editable: false, sortable: false, filterable: false, columntype: 'button', width: '5%', align: 'center', cellsrenderer: function () {
						return "Edit";
						}, buttonclick: function (row) {
							editrow 		= row;
					...
					{ text: 'Send', editable: false, sortable: false, filterable: false, columntype: 'button', width: '5%', align: 'center', cellsrenderer: function () {
						return "Send";
						}, buttonclick: function (row) {		
							editrow = row;	
							var offset 		= $("#gridsuratkeluar").offset();		
					...
					{ text: 'Approval', editable: false, sortable: false, filterable: false, columntype: 'button', width: '8%', align: 'center', cellsrenderer: function () {
						return "Action";
							...
							if (jenissrt == 'REQUEST'){
								$("#reqnomor_idne").val(dataRecord.id);
								$("#reqnomor_kodepjbt").val(dataRecord.kodefak);
								$("#reqnomor_nomor").val(dataRecord.nomor);
								$("#reqnomor_tanggal").val(dataRecord.plaintglsurat);
								$("#reqnomor_perihal").val(dataRecord.plainperihal);
								$("#reqnomor_pemohon").val(dataRecord.pembuat);
								$("#reqnomor_jenissrt").val('UPLQRMAN');
								$("#modalaprovalnomor").modal('show');
							} else {
								swal({
									title	: 'Stop',
									text	: 'Tombol ini Khusus untuk Approval Nomor Yang diminta oleh Staf Selain Sekretaris dan Admin SDM',
									type	: 'error',
								})
							}
	...
	//KELOMPOK_EVENT
		...
		$("#btnapprovereqnomor").click(function(){
			var val01=document.getElementById('reqnomor_idne').value;
			var val02=document.getElementById('reqnomor_jenissrt').value;
			var val03=document.getElementById('reqnomor_kodepjbt').value;
			var token=document.getElementById('token').value;
			if (val01 == '' || val02 == '' || val03 == ''){
				swal({
					title	: 'Stop',
					text	: 'Jenis Surat dan Penandatangan Tidak Boleh Kosong',
					type	: 'error',
				})
			} else {
				$.post('webinar/saveeditemail', { set01: val01, set02: val02, set03: val03, _token: token },
	...
	//KELOMPOK_AWAL_KALENDER
		...
		$("#btnopenmailarsip").click(function(){
			...
			var sumbersuratmasukcari = {
				...
				data		: {dari:'', jenis:'arsip', surat:'MASUK', _token: '{{ csrf_token() }}'},
				type		: 'POST',
				url			: '{{ route("inboxSuratmasukPaged") }}',
				root		: 'data',
				totalrecords: 'total',
			...
			$("#gridsendiricari").jqxGrid({
				...
				altrows			: true,
				columns			: [
					{ text: 'Surat', editable: false, sortable: false, filterable: false, columntype: 'button', width: '8%', cellsrenderer: function () {
						return "Preview";
						}, buttonclick: function (row) {
							editrow = row;	
							var offset 		= $("#gridsendiricari").offset();		
							var dataRecord 	= $("#gridsendiricari").jqxGrid('getrowdata', editrow);
							var set03		= '{{URL::to("/")}}/viewsurat/7a07275b47504815818abc970da769fc-'+dataRecord.idsurat;
					...
					{ text: 'No.Surat', datafield: 'nosurat', width: '8%', cellsalign: 'left', align: 'center'  },
					{ text: 'Perihal', datafield: 'perihal', width: '25%', cellsalign: 'left', align: 'center'  },
					{ text: 'Disposisi', datafield: 'footnote', width: '25%', cellsalign: 'left', align: 'center'  },
					{ text: 'Kepada', datafield: 'kepada', width: '20%', cellsalign: 'left', align: 'center'  },
					{ text: 'Timestamp', datafield: 'created_at', width: '7%', cellsalign: 'center', align: 'center'  },
