...
class AdminController extends Controller
{
	public function exSavesekolah(Request $request) {
		...
		if ($request->hasFile('logo')) {
			...
			if ($ekstensi != 'png') {
				...
				$uploadedFile->move(public_path('logo/'), $namafile);
				// File::copy(public_path('/').$namafile, public_path('logo/').$namafile);
			...
			if ($request->hasFile('logo_grey')) {
				...
				if ($ekstensi != 'png') {
					...
					$uploadedFile->move(public_path('logo/'), $background);
			...
			if ($request->hasFile('frontpage')) {
				...
				if ($ekstensi != 'png') {
					...
					$uploadedFile->move(public_path('logo/'), $frontlogo);
	...
	public function exUpdatesekolah(Request $request) {
		$update 	= Sekolah::find($request->edit_id)->update([
	...
	public function viewAmilZIS() {
		...
		$tasks['logo01']  			= Session('sekolah_logo');
		if (Session('previlage') == 'level1' OR Session('previlage') == 'adminzis' OR Session('previlage') == 'level3'){
	...
	public function viewDatainduk() {
		...
		$tasks['logo']				= Session('sekolah_logo');
		...
		if (Session('previlage') == 'level1' OR Session('previlage') == 'level3'){
	...
	public function viewLogkeuangan() {
		...
		$groups 	= Logstaff::select(DB::Raw('DATE(timestamp) day'))
						->where('sopo', '!=', 'Perubahan Nilai')
		...
		foreach ($groups as $group) {
			$tanggal    = $group->day;
			$rsurat     = Logstaff::where('timestamp', 'like', '%'. $tanggal . '%')->where('sopo', '!=', 'Perubahan Nilai')->orderBy('id', 'DESC')->get();
			foreach ($rsurat as $rowpeng) {
				$siapa          = $rowpeng->sopo;
				$pengumuman     = $rowpeng->kelakuan;   
				$created_at     = $rowpeng->timestamp;
				$kapan          = timeAgoA($created_at);
				$iconne			= 'fa-bullhorn';
				$jencolor 		= 'red';
	...
	public function viewLapppdb() {
		...
		$tasks['pendaftaran']		= $rsetting->pendaftaran;
		$tasks['pengumuman']		= $rsetting->pengumuman;
		$tasks['tahunne']			= date("Y");
		$tasks['tanggal']			= date("Y-m-d");
		$tasks['namaapps01']  		= Session('sekolah_nama_aplikasi');
		$tasks['domainapps01']  	= Session('sekolah_nama_yayasan');
	...
	public function viewPengumuman() {
		...
		} else if (Session('subsubdomainapps01') !== null){
			$groups     =   Pengumuman::where('fakultas', Session('subsubdomainapps01'))->select('tanggal')->groupBy('tanggal')->orderBy('tanggal', 'DESC')->limit(30)->get();
		} else {
			$groups     =   Pengumuman::where('fakultas', $subsubdomainapps01)->select('tanggal')->groupBy('tanggal')->orderBy('tanggal', 'DESC')->limit(30)->get();
		}
		$y      	=   0;
		$x      	=   0;
		$tasks   	=   [];
		foreach ($groups as $group) {
			...
			if (Session('sekolah_id_sekolah') !== null){
				...
				$rsurat     = Pengumuman::where('fakultas', Session('subsubdomainapps01'))->where('tanggal', 'like', '%'. $tanggal . '%')->orderBy('id', 'DESC')->limit(30)->get();
			} else {
				$rsurat     = Pengumuman::where('fakultas', $subsubdomainapps01)->where('tanggal', 'like', '%'. $tanggal . '%')->orderBy('id', 'DESC')->limit(30)->get();
			}
			foreach ($rsurat as $rowpeng) {
				$id             =   $rowpeng->id;
				$jenis          =   $rowpeng->jenis;
	...
	public function viewSetting() {
		...
		$rsekolah						= Sekolah::find(session('sekolah_id_sekolah'));
		$tasks							= [];
	...
	public function viewLaporan() {
		...
		$data['logofrontapps01']  	= Session('sekolah_frontpage');
		$data['logo01']  			= Session('sekolah_logo');
		if (Session('previlage') == 'level1' OR Session('previlage') == 'level3' OR Session('spesial') == 'paguyuban'){
	...
	public function exLaporanbulanan(Request $request) {
		...
		if ($bulan != '' and $tahun != '' and $jenis != '' and $tanggal != '' and $tgllaporan != ''){
			...
			if ($jenis == 'operasional') { $tulisanne = '(01). LAPORAN OPERASIONAL RUTIN<br />PERIODE BULAN '.$blniki.' TAHUN '.$tahun; }
			if ($jenis == 'spp') { $tulisanne = '(02). LAPORAN KEUANGAN PEMBAYARAN SPP<br />PERIODE BULAN '.$blniki.' TAHUN '.$tahun; }
			if ($jenis == 'dpp') { $tulisanne = '(03). LAPORAN KEUANGAN DANA PEMBANGUNAN<br />PERIODE BULAN '.$blniki.' TAHUN '.$tahun; }
			if ($jenis == 'bos') { $tulisanne = '(04). LAPORAN KEUANGAN DANA BOS<br />PERIODE BULAN '.$blniki.' TAHUN '.$tahun; }
			if ($jenis == 'pajak') { $tulisanne = '(05). LAPORAN KEUANGAN PAJAK<br />PERIODE BULAN '.$blniki.' TAHUN '.$tahun; }
			if ($jenis == 'nonopsrutin') { $tulisanne = '(06). LAPORAN NON OPERASIONAL RUTIN <br />PERIODE BULAN '.$blniki.' TAHUN '.$tahun; }
			if ($jenis == 'lainlain') { $tulisanne = '(07). LAPORAN KEUANGAN LAIN-LAIN <br />PERIODE BULAN '.$blniki.' TAHUN '.$tahun; }
			echo <<<EOM
			...
			foreach($golekjeneng as $hasil){
				...
				if ($itung == 16){
					...
					echo <<<EOM
					  ...
					  <tr>
						<td valign="top" style="border-left:thin; border-right:thin;" align="center">$i</td>
						<td valign="top" style="border-right:thin;" align="center">$tanggal</td>
						<td valign="top" style="border-right:thin;">$deskripsi</td>
						<td valign="top" width="8%" align="center">Rp.</td>
						<td valign="top" width="16%" style="border-right:thin;" align="right">$tulisan4</td>
						<td valign="top" width="8%" align="center">Rp.</td>
						<td valign="top" width="16%" style="border-right:thin;" align="right">$tulisan5</td>
						<td valign="top" width="8%" align="center">Rp.</td>
						<td valign="top" width="16%" style="border-right:thin;" align="right">$tulisan6</td>
					  </tr>
					   <tr>
						<td valign="top" style="border-left:thin; border-right:thin;">&nbsp;</td>
						<td valign="top" style="border-right:thin;">&nbsp;</td>
					  ...
					  <tr>
						...
						<td valign="top" width="17%" style="border-bottom:double; border-top:double; border-right:thin;" align="right">$tulisan7</td>
						<td valign="top" width="2%" style="border-bottom:double; border-top:double; border-right:thin;" align="center">Rp.</td>
						<td valign="top" width="17%" style="border-bottom:double; border-top:double; border-right:thin;" align="right">$tulisan8</td>
						<td valign="top" width="3%" style="border-bottom:double; border-top:double; border-right:thin;" align="center">Rp.</td>
						<td valign="top" width="27%" style="border-bottom:double; border-top:double; border-right:thin;" align="right">$tulisan6</td>
					  </tr>
					</table>
...
EOM;
		...
		else { echo '<h2>Mohon Lengkapi Komponennya</h2>'; }
	}
	...
	public function exKwitansiByID($id) {
		if (file_exists(public_path('kwitansi/'.$id.'.pdf'))){
			...
			if (isset($getdata->id)){
				...
				$rsetting		= Sekolah::find($id_sekolah);
				if (isset($rsetting->id)){
					...
					$logogrey 		= $homebase.'/boxed-bg.png';	
				}
				$x 				= TerbilangA($total);
	...
	public function getRekapsaldo() {
		...
		if ($ceksek == 0){
			$jenislist 	= array(0 => "lainlain", 1 => "Paguyuban", 2 => "Ortu_Asuh", 3 => "Bazar", 4 => "Rihlah", 5 => "Tahsin", 6 => "Dana Kesiswaan", 7 => "operasional", 8 => "spp", 9 => "dpp", 10 => "bos", 11 => "pajak", 12 => "nonopsrutin", 13 => "PHBI", 14 => "Sedekah Rutin", 15 => "DanSosOp");
		...
		foreach($getdata as $hasil){
			...
			$saldo		= $hasil->saldo;
			$tlsjenis	= '';
			if ($jenis == 'operasional') { $tulisanne = '01. LAPORAN OPERASIONAL RUTIN'; }
			elseif ($jenis == 'spp') { $tulisanne = '02. LAPORAN KEUANGAN PEMBAYARAN SPP'; }
			elseif ($jenis == 'dpp') { $tulisanne = '03. LAPORAN KEUANGAN DANA PEMBANGUNAN'; }
			elseif ($jenis == 'bos') { $tulisanne = '04. LAPORAN KEUANGAN DANA BOS'; }
			elseif ($jenis == 'pajak') { $tulisanne = '05. LAPORAN KEUANGAN PAJAK'; }
			elseif ($jenis == 'nonopsrutin') { $tulisanne = '06. LAPORAN NON OPERASIONAL RUTIN '; }
			elseif ($jenis == 'lainlain') { $tulisanne = '07. LAPORAN KEUANGAN LAIN-LAIN '; }
					...
					if ($input){
						if ($request->hasFile('edit_foto')) {
							...
							if ($validator->fails()) {
								...
								$uploadedFile->move(public_path('dist/img/foto/'), $namafile);
	...
	public function jsonCariDatainduk(Request $request) {
		...
		$getallsiswa 	= Datainduk::where('tamasuk', $valcari)->orderBy('noinduk', 'DESC')->where('id_sekolah',session('sekolah_id_sekolah'))->get();
		if (!empty($getallsiswa)){
			foreach ($getallsiswa as $hasil) {
				$foto 		= $hasil->foto;
				if ($foto != ''){
	...
	public function jrekapthnini() {
		...
		$maal		= 0;
		$shodaqoh	= 0;
		$getallthn 	= Pembayaranzis::where('created_at', 'LIKE', $tahun.'%')->where('tglvalidasi', '!=', '0000-00-00')->orderBy('updated_at', 'ASC')->where('id_sekolah',session('sekolah_id_sekolah'))->get();
		if (!empty($getallthn)){
			foreach ($getallthn as $rdatane) {
				$jeniszakat		= $rdatane->jeniszakat;
				$nominal		= $rdatane->nominal;
	...
	public function viewLampiran ($id){
		...
		$data['logofrontapps01']  	= Session('sekolah_frontpage');
		$data['logo01']  			= Session('sekolah_logo');
		$cekdata 					= Pembayaranzis::where('id', $id)->where('id_sekolah',session('sekolah_id_sekolah'))->count();
		if ($cekdata == 0){
			$data['gambar']   		= 'dist/img/takadagambar.png';
		} else {
			$getdata 				= Pembayaranzis::where('id', $id)->where('id_sekolah',session('sekolah_id_sekolah'))->first();
			$namafile				= $getdata->namafile;
			$data['gambar']   		= $namafile;
		}
		return view('filelampiran', $data);
	}
	...
	public function jsonEkskul() {
		...
		if (!empty($getalekskul)){
			foreach ($getalekskul as $hasil) {
				$set01		= $hasil->nama;
				$jumlah 	= DB::table('db_setkeuangan')->where('db_setkeuangan.id_sekolah',session('sekolah_id_sekolah'))
								->join('db_datainduk', 'db_setkeuangan.noinduk', 'db_datainduk.noinduk')
								->where('db_datainduk.nokelulusan', '')
								->where('db_setkeuangan.eksul1', 'LIKE', $set01)
								->orWhere('db_setkeuangan.eksul2', 'LIKE', $set01)
								->orWhere('db_setkeuangan.eksul3', 'LIKE', $set01)
								->orWhere('db_setkeuangan.eksul4', 'LIKE', $set01)
								->orWhere('db_setkeuangan.eksul5', 'LIKE', $set01)
								->count();
				$arrekskul[] = array(
					'id' 		=> $hasil->id,
	...
	public function jsonSetkeuangan(Request $request) {
		$kelas   	=   $request->val01;
		if ($kelas == 'all'){
			$getallsiswa=	Datainduk::where('nokelulusan', '')->where('id_sekolah',session('sekolah_id_sekolah'))->get();
		} else {
			$getallsiswa=	Datainduk::where('klspos', 'LIKE', $kelas.'%')->where('nokelulusan', '')->where('id_sekolah',session('sekolah_id_sekolah'))->get();
		}
		$homebase	= url("/");
		$alldata 	= [];
		if (!empty($getallsiswa)){
			foreach ($getallsiswa as $hasil) {
				...
				if (File::exists(base_path()) ."/public/dist/img/foto/". $foto) {
					...
					$lampiran	= '<img src="'.$homebase.'/wasimong.png" height="35">';
				}	
	...
	public function exEditorbyr(Request $request) {
		...
		if($alasan == '' or $biaya == ''){
			...
			$rlampau	= Pembayaran::where('id', $idne)->where('id_sekolah',session('sekolah_id_sekolah'))->first();
			$jenis 		= $rlampau->jenis;
			...
			$update		= Pembayaran::where('id', $idne)->where('id_sekolah',session('sekolah_id_sekolah'))->update([
				'biaya'		=> $biaya,
				'inputor'	=> $sopo,				
				'id_sekolah'=> session('sekolah_id_sekolah'),				
			]);
	...
	public function exUploaddatainduk(Request $request) {
		if ($request->hasFile('datainduk')) {
			...
			foreach($getalldata as $val){
				...
				if ($val['T'] == ''){ $foto = 'wasimong.png'; }else { $foto = $val['T']; }
	...
	public function exSimpandataujian(Request $request) {
		...
		if ($hari == '' OR $jam == '' OR $materi == '' OR $tanggal == '' OR $ruang == '' OR $nama == ''){
			...
			if ($idne == 'new'){
				...
				$simpan	= Tesppdb::where('id', $idne)->where('id_sekolah',session('sekolah_id_sekolah'))->update([
					'hari'		=> $hari, 
					'jam'		=> $jam, 
					'materi'	=> $materi, 
					'nama'		=> $nama, 
	...
	public function ctkKwitansipsb(Request $request) {
		...
		if ($idne != ''){
			...
			$rmaster2	= Datapsb::where('id', $idne)->where('id_sekolah',session('sekolah_id_sekolah'))->first();
			$nama 		= $rmaster2->nama;
			$tamasuk	= $rmaster2->tamasuk;
			$costumid	= $rmaster2->kodependaf;
			
			$nominal	= $hargaformulir;
			$nomhuruf 	= TerbilangA($nominal);
		...
		$alamatcetak			= $homebase.'/kwitansipsb/'.$costumid;
		$qrcode 				= QrCode::size(75)->generate($alamatcetak);
		$tasks					= [];
	...
	public function exSavearsipppdb(Request $request) {
		...
		if ($cekmasuk != 0){
			...
			if ($input){
				Datapsb::where('id', $idne)->where('id_sekolah',session('sekolah_id_sekolah'))->update([
					'tamasuk' => $arsip
				]);
				echo '<div class="alert alert-success alert-dismissable">
						<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
	...
	public function exSaveverifikasipsb(Request $request) {
		...
		if ($verifikasi == ''){
			...
			$input 	= Datapsb::where('id', $idne)->where('id_sekolah',session('sekolah_id_sekolah'))->update([
				'status' => $verifikasi
			]);
			if ($input){
				echo '<div class="alert alert-success alert-dismissable">
	...
	public function exSavesettingssppdpp(Request $request) {
		...
		$status	= '';
		//nominal1
		$cek1 	= Layanan::where('layanan', 'spp1')->where('id_sekolah',session('sekolah_id_sekolah'))->count();
		if ($cek1 == 0){
			Layanan::create([
				'layanan'	=> 'spp1', 
				'status'	=> $spp1,
			...
			$status = $status.'<br />Tambah Setting SPP 1 Nominal '.$request->val01;
		} else {
			Layanan::where('layanan', 'spp1')->where('id_sekolah',session('sekolah_id_sekolah'))->update([
				'status'	=> $spp1,
				'id_sekolah'=> session('sekolah_id_sekolah'),
			]);
			$status = $status.'<br />Update Setting SPP 1 Nominal '.$request->val01;
		}
		//nominal2 
		$cek2 	= Layanan::where('layanan', 'spp2')->where('id_sekolah',session('sekolah_id_sekolah'))->count();
		if ($cek2 == 0){
			Layanan::create([
				'layanan'	=> 'spp2', 
				'status'	=> $spp2,
			...
			$status = $status.'<br />Tambah Setting SPP 2 Nominal '.$request->val02;
		} else {
			Layanan::where('layanan', 'spp2')->where('id_sekolah',session('sekolah_id_sekolah'))->update([
				'status'	=> $spp2,
				'id_sekolah'=> session('sekolah_id_sekolah'),
			]);
			$status = $status.'<br />Update Setting SPP 2 Nominal '.$request->val02;
		}
		//nominal3 
		$cek3 	= Layanan::where('layanan', 'spp3')->where('id_sekolah',session('sekolah_id_sekolah'))->count();
		if ($cek3 == 0){
			Layanan::create([
				'layanan'	=> 'spp3', 
				'status'	=> $spp3,
			...
			$status = $status.'<br />Tambah Setting SPP 3 Nominal '.$request->val03;
		} else {
			Layanan::where('layanan', 'spp3')->update([
				'status'	=> $spp3,
				'id_sekolah'=> session('sekolah_id_sekolah'),
			]);
			$status = $status.'<br />Update Setting SPP 3 Nominal '.$request->val03;
		}
		//dpp1 
		$cek4 	= Layanan::where('layanan', 'dpp1')->where('id_sekolah',session('sekolah_id_sekolah'))->count();
		if ($cek4 == 0){
			Layanan::create([
				'layanan'	=> 'dpp1', 
				'status'	=> $dpp1,
			...
			$status = $status.'<br />Tambah Setting DPP 1 Nominal '.$request->val04;
		} else {
			Layanan::where('layanan', 'dpp1')->where('id_sekolah',session('sekolah_id_sekolah'))->update([
				'status'	=> $dpp1,
			]);
			$status = $status.'<br />Update Setting DPP 1 Nominal '.$request->val04;
		}
		//dpp2 
		$cek5 	= Layanan::where('layanan', 'dpp2')->where('id_sekolah',session('sekolah_id_sekolah'))->count();
		if ($cek5 == 0){
			Layanan::create([
				'layanan'	=> 'dpp2', 
				'status'	=> $dpp2,
			...
			$status = $status.'<br />Tambah Setting DPP 2 Nominal '.$request->val05;
		} else {
			Layanan::where('layanan', 'dpp2')->where('id_sekolah',session('sekolah_id_sekolah'))->update([
				'status'	=> $dpp2,
				'id_sekolah'=> session('sekolah_id_sekolah'),
			]);
			$status = $status.'<br />Update Setting DPP 2 Nominal '.$request->val05;
		}
		echo '<div class="alert alert-info alert-dismissable">
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
				<h4><i class="icon fa fa-check"></i> Sukses!</h4>
	...
	public function exSavesettingppdb(Request $request) {
		...
		$status			= '';
		Sekolah::find(session('sekolah_id_sekolah'))->update([
			'pengumuman' 	=> $pengumuman,
			'pendaftaran' 	=> $tapel,
		]);
		$cek1 		= Layanan::where('layanan', 'ppdb')->where('id_sekolah',session('sekolah_id_sekolah'))->count();
		if ($cek1 == 0){
			Layanan::create([
				'layanan'	=> 'ppdb', 
				'status'	=> $status,
			...
			$status = $status.'<br />Tambah Setting ppdb '.$request->val02;
		} else {
			Layanan::where('layanan', 'ppdb')->where('id_sekolah',session('sekolah_id_sekolah'))->update([
				'status'	=> $status,
				'id_sekolah'=>session('sekolah_id_sekolah')
			]);
			$status = $status.'<br />Update Setting ppdb '.$request->val02;
		}
		$cek2 		= Layanan::where('layanan', 'kodebaru')->where('id_sekolah',session('sekolah_id_sekolah'))->count();
		if ($cek2 == 0){
			Layanan::create([
				'layanan'	=> 'kodebaru', 
				'status'	=> $kodebaru,
			...
			$status = $status.'<br />Tambah Setting Kode Siswa Baru '.$request->val04;
		} else {
			Layanan::where('layanan', 'kodebaru')->where('id_sekolah',session('sekolah_id_sekolah'))->update([
				'status'	=> $kodebaru,
				'id_sekolah'=>session('sekolah_id_sekolah')
			]);
			$status = $status.'<br />Update Setting Kode Siswa Baru '.$request->val04;
		}
		$cek3 		= Layanan::where('layanan', 'kodepindahan')->where('id_sekolah',session('sekolah_id_sekolah'))->count();
		if ($cek3 == 0){
			Layanan::create([
			...
			$status = $status.'<br />Tambah Setting Kode Siswa Pindahan '.$kodepindahan;
		} else {
			Layanan::where('layanan', 'kodepindahan')->where('id_sekolah',session('sekolah_id_sekolah'))->update([
				'status'	=> $kodepindahan,
				'id_sekolah'=>session('sekolah_id_sekolah')
			]);
			$status = $status.'<br />Update Setting Kode Siswa Pindahan '.$kodepindahan;
		}
		
		$cek4 		= Layanan::where('layanan', 'hargaformulir')->where('id_sekolah',session('sekolah_id_sekolah'))->count();
		if ($cek4 == 0){
			Layanan::create([
			...
			$status = $status.'<br />Tambah Setting Harga Formulirpsb '.$request->val07;
		} else {
			Layanan::where('layanan', 'hargaformulir')->where('id_sekolah',session('sekolah_id_sekolah'))->update([
				'status'	=> $hargaformulir,
			]);
			$status = $status.'<br />Update Setting Harga Formulirpsb '.$request->val07;
		}
		
		$cek5 		= Layanan::where('layanan', 'namabank')->where('id_sekolah',session('sekolah_id_sekolah'))->count();
		if ($cek5 == 0){
			Layanan::create([
				'layanan'	=> 'namabank', 
				'status'	=> $namabank,
			]);
			$status = $status.'<br />Tambah Setting Nama Bank :  '.$request->val08;
		} else {
			Layanan::where('layanan', 'namabank')->where('id_sekolah',session('sekolah_id_sekolah'))->update([
				'status'	=> $namabank,
				'id_sekolah'=>session('sekolah_id_sekolah')
			]);
			$status = $status.'<br />Update Setting Nama Bank :  '.$request->val08;
		}
		
		$cek6 		= Layanan::where('layanan', 'norek')->where('id_sekolah',session('sekolah_id_sekolah'))->count();
		if ($cek6 == 0){
			Layanan::create([
				'layanan'	=> 'norek', 
				'status'	=> $norek,
			...
			$status = $status.'<br />Tambah Setting Norek Bank :  '.$request->val09;
		} else {
			Layanan::where('layanan', 'norek')->where('id_sekolah',session('sekolah_id_sekolah'))->update([
				'status'	=> $norek,
				'id_sekolah'=>session('sekolah_id_sekolah')
			]);
			$status = $status.'<br />Update Setting Norek Bank :  '.$request->val09;
		}
		
		$cek6 		= Layanan::where('layanan', 'periodepsb')->where('id_sekolah',session('sekolah_id_sekolah'))->count();
		if ($cek6 == 0){
			Layanan::create([
				'layanan'	=> 'periodepsb', 
				'status'	=> $periodepsb,
			...
			$status = $status.'<br />Tambah Setting Periode PSB :  '.$request->val10;
		} else {
			Layanan::where('layanan', 'periodepsb')->where('id_sekolah',session('sekolah_id_sekolah'))->update([
				'status'	=> $periodepsb,
				'id_sekolah'=>session('sekolah_id_sekolah')
			]);
			$status = $status.'<br />Update Setting Periode PSB :  '.$request->val10;
		}
		echo '<div class="alert alert-info alert-dismissable">
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
				<h4><i class="icon fa fa-check"></i> Sukses!</h4>
	...
	public function jsonDatappdb() {
		...
		$bulanlist 	= array(1 => "Januari", 2 => "Februari", 3 => "Maret", 4 => "April", 5 => "Mei", 6 => "Juni", 7 => "Juli", 8 => "Agustus", 9 => "September", 10 => "Oktober", 11 => "November", 12 => "Desember");
		$sql 		= Datapsb::where('tamasuk', $pendaftaran)->where('id_sekolah',session('sekolah_id_sekolah'))->orderBy('id', 'DESC')->get();
		if (!empty($sql)){
			foreach ($sql as $hasil){
				$setstatus 	= $hasil->status;
				if ($setstatus == 'verified'){
					$status = '<small class="label bg-green">GOOD</small>';
				} else if ($setstatus == 'unverified'){
					$status = '<small class="label bg-red">BAD</small>';
				} else {
					$status = '<strong>'.$setstatus.'%</strong>';
				}
				$arrdatappdb[] = array(
					'id' 			=> $hasil->id,
					'nextid' 		=> $berikutnya,
					'persenselesai' => $status,
					'nama' 			=> $hasil->nama,
					'nik' 			=> $hasil->nik,
					'kodependaf' 	=> $hasil->kodependaf,
					'kelamin' 		=> $hasil->kelamin,
					'tmplahir' 		=> $hasil->tmplahir,
					'tgllahir' 		=> $hasil->tgllahir,			
					'darah' 		=> $hasil->darah,
	...
	public function jsonBuku() {
		...
		if ($i == 0){
			$sql 		= Perpumini::orderBy('id', 'DESC')->where('id_sekolah', '1')->groupby('isbn')->get();
			if (!empty($sql)){
				foreach ($sql as $hasil){
					Perpumini::create([
						'gambar' 		=> $hasil->gambar,		
	...
	public function jsonPeminjaman(Request $request) {
		...
		$tahun		= $request->val02;
		if ($bulan == 'aktif'){
			$sql 		= Peminjaman::where('id_sekolah',session('sekolah_id_sekolah'))->where('status','1')->orderBy('tglkembali', 'ASC')->get();
		} else {
			if ($bulan == 'ALL'){
				$sql 		= Peminjaman::where('id_sekolah',session('sekolah_id_sekolah'))->where('tglpinjam','LIKE', $tahun.'%')->orderBy('tglkembali', 'ASC')->get();
			} else {
				$sql 		= Peminjaman::where('id_sekolah',session('sekolah_id_sekolah'))->where('tglpinjam','LIKE', $tahun.'-'.$bulan.'-%')->orderBy('tglkembali', 'ASC')->get();		
			}
		}
		if (!empty($sql)){
			foreach ($sql as $hasil){
				...
				} else { $tlsstatus = '<span class="badge bg-aqua">AKTIF</span>'; }
				...
				} else { $gambar = ''; $kodebuku = $idbuku; $rakbuku = ''; $judul = ''; $link = ''; }

				if (is_null($gambar) OR $gambar == ''){ $lampiran	= '<img src="'.$homebase.'/logo.png" height="35">'; }
				else {
					if (File::exists(base_path()) ."/public/images/perpus/". $gambar) {
						$lampiran	= '<img src="'.$homebase.'/images/perpus/'.$gambar.'" height="35">';
					} else {
						$lampiran	= '<img src="'.$homebase.'/logo.png" height="35">';
					}
				}
				$getfoto 	= Datainduk::where('noinduk', $hasil->noinduk)->first();
				if (isset($getfoto->foto)){
					$foto 		= $getfoto->foto;
					if ($foto != ''){
						if (File::exists(base_path()) ."/public/dist/img/foto/". $foto) {
				...
				} else {$foto	= '<img src="'.$homebase.'/logo.png" height="35">';}
		...
		if ($gakboleh != ''){
			...
			if ($input){
				if ($request->hasFile('file')) {
					...
					$request->file('file')->move(public_path('images/perpus/'), $namafile);
	...
	public function exDestroyer(Request $request) {
		...
		if ($tabel == 'datanilai'){
			$valcari 	= '';
			$getdata 	= Loginputnilai::where('marking', $idne)->first();
			if (isset($getdata->id)){
				$jumlah 	= Datanilai::where('marking', 'LIKE', $idne.'%')->where('id_sekolah', session('sekolah_id_sekolah'))->count();
				...
				Logstaff::create([
					'sopo'			=> Session('nama'), 
					'kelakuan'		=> 'Menghapus Data Nilai '.$valcari,
					'id_sekolah' 	=> session('sekolah_id_sekolah')
				]);
			}
			$busek = Loginputnilai::where('marking', $idne)->delete();
		...
		if ($busek){
			if ($tabel == 'perpusmini' OR $tabel == 'db_programpip'){
				Logstaff::create([
					'sopo'		=> Session('nama'), 
					'kelakuan'	=> $deskripsi,
					'id_sekolah'=> session('sekolah_id_sekolah')
	...
	public function exSavesetting(Request $request) {
		$update 	= Sekolah::find(session('sekolah_id_sekolah'))->update([
			'nama_yayasan' 		=> $request->edit_nama_yayasan, 
			'nis' 				=> $request->edit_nis, 
			'nss' 				=> $request->edit_nss, 
			'npsn' 				=> $request->edit_npsn, 
	...
	public function exKendaraan(Request $request) {
		...
		if ($namarg == 'hapus') {
			...
			$kelakuan 	= 'Menghapus data ruang '.$nama.' Kode Ruang : '.$kode.' Unit : '.$fakultas;
			Logstaff::create([
				'sopo'			=> Session('nama'), 
				'kelakuan'		=> $kelakuan,
				'id_sekolah' 	=> session('sekolah_id_sekolah')
			...
			if ($cekfas == 0){				
				$kelakuan 	= 'Menghapus data Garasi '.$namagd.' Kode : '.$kodegd.' Unit : '.$fakultas;
				Logstaff::create([
					'sopo'			=> Session('nama'), 
					'kelakuan'		=> $kelakuan,
					'id_sekolah' 	=> session('sekolah_id_sekolah')
					...
					if ($wajibada == 1 AND $pjgedung == '0'){
						...
						if ($input){
							if ($request->hasFile('file')) {
								...
								$request->file->move(public_path('images/kendaraan'), $namafile);
	...
	public function getallruang() {
		$arrayruang = [];
    	$jruang 	= Ruang::where('fakultas', Session('sekolah_id_sekolah'))->orderBy('kodegd', 'asc')->orderBy('namarg', 'asc')->get();
    	foreach ($jruang as $result) {
    ...
    public function getallgedung() {
		$arrayruang = [];
		$fakultas	= Session('sekolah_id_sekolah');
    	$jruang 	= Gedung::where('fakultas', Session('sekolah_id_sekolah'))->orderBy('namagd', 'asc')->get();
    	foreach ($jruang as $result) {
			$pjgedung 	= $result->pjgedung;
			$foto 		= $result->foto;
			if (is_null($foto)){ $foto = '';}
	...
	public function exruang(Request $request) {		
		...
		if ($namarg == 'hapus') {			
			...
			if ($cekfas == 0){				
				$kelakuan 	= 'Menghapus data Gedung '.$namagd.' Kode : '.$kodegd.' Unit : '.$fakultas;
				Logstaff::create([
					'sopo'			=> Session('nama'), 
					'kelakuan'		=> $kelakuan,
					'id_sekolah' 	=> session('sekolah_id_sekolah')
					...
					if ($wajibada == 1 AND $pjgedung == '0'){
						...
						if ($input){
							if ($request->hasFile('file')) {								
								...
								$namafile		= $namafile.'.'.$request->file->getClientOriginalExtension();
								$request->file->move(public_path('images/gedung'), $namafile);
					...
					if ($wajibada == 1 AND $pjgedung == '0'){
						...
						if ($input){
							if ($request->hasFile('file')) {
								...
								$namafile		= $namafile.'.'.$request->file->getClientOriginalExtension();
								$request->file->move(public_path('images/ruang'), $namafile);
	...
	public function exSimpanprestasi(Request $request) {
		...
		if ($noinduk == 'hapus'){
			...
			$namafile	= $getdata->namafile;
			if ($namafile != ''){
				if (File::exists(base_path()) ."/public/dist/sertifikat/". $namafile) {
				  File::delete(base_path() ."/public/dist/sertifikat/". $namafile);
				}
			}
			$hapus 	= Prestasi::where('id', $idne)->delete();
			if ($hapus){
				$deskripsi 	= 'Menghapus data Prestasi '.$getdata->nama.' No.Induk : '.$getdata->noinduk.' Pada Lomba/Kegiatan '.$getdata->namakegiatan;
				Logstaff::create([
					'sopo'		=> Session('nama'),
					'kelakuan'	=> $deskripsi,
				]);
					...
					if ($cekdata != 0){
						...
						if ($input){
							...
							if ($request->hasFile('file')) {
								...
								$request->file('file')->move(public_path('dist/img/sertifikat/'), $namafile);
	...
	public function jsonAlldataprestasi(Request $request) {
		...
		if ($bulan == 'REGIONAL'){
			$getallthn 	= Prestasi::where('id_sekolah', Session('sekolah_id_sekolah') )->where('tanggal', 'LIKE', '%'.$tahunini.'%')->where('tingkat', 'Regional')->get();
		} else if($bulan == 'NASIONAL'){
			$getallthn 	= Prestasi::where('id_sekolah', Session('sekolah_id_sekolah') )->where('tanggal', 'LIKE', '%'.$tahunini.'%')->where('tingkat', 'Nasional')->get();
		} else if($bulan == 'INTERNASIONAL'){
			$getallthn 	= Prestasi::where('id_sekolah', Session('sekolah_id_sekolah') )->where('tanggal', 'LIKE', '%'.$tahunini.'%')->where('tingkat', 'Internasional')->get();
		} else if($bulan == 'ALL'){
			if ($tahun == 'TAHUNINI'){
				$getallthn 	= Prestasi::where('id_sekolah', Session('sekolah_id_sekolah') )->where('tanggal', 'LIKE', '%'.$tahunini.'%')->get();
			} else {
				$getallthn 	= Prestasi::where('id_sekolah', Session('sekolah_id_sekolah') )->where('tanggal', 'LIKE', '%'.$tahun.'%')->get();
			}
			
		} else {
			$getallthn 	= Prestasi::where('id_sekolah', Session('sekolah_id_sekolah') )->where('tanggal', 'LIKE', '%'.$bulan.'-'.$tahun.'%')->get();
		}
		$alldata 	= [];
		$homebase	= url("/");
		if (!empty($getallthn)){
